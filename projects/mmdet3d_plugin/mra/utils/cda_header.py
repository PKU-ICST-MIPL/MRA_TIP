...
class CriticalDisttributionAlignmentHead(nn.Module):
    ...
    def forward(self, x, voxel_feature):
        output_dict = {}
        ...
        sampled_voxels_coords = sampling_critical_voxels(coarse_prediction, self.N, self.t, self.omega)
...
@torch.no_grad()
def sampling_critical_voxels(mask, N, t=3, omega=0.75):
    device = mask.device
    ...
    critical_score = p_a
    _, voxel_index = critical_score.topk(int(omega * N), dim=1)
    voxels_critical_sampled = random_sampled_coords.view(-1, 3)[voxel_index.view(-1), :].view(B, int(omega * N), 3)

    voxels_random_sampled = torch.rand(B, N - int(omega * N), 3, device=device)
    return torch.cat([voxels_critical_sampled, voxels_random_sampled], 1).to(device)

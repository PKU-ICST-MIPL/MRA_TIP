...
class FLoSP(nn.Module):
    ...
    def forward(self, mlvl_feats, img_metas, down_scale=1):
        ...
        try:
            projected_pix = projected_pix.reshape(5, self.bev_h, self.bev_w, self.bev_z, 2)
            fov_mask = fov_mask.reshape(5, self.bev_h, self.bev_w, self.bev_z)
            projected_pix = projected_pix[:, ::down_scale, ::down_scale, ::down_scale, :].reshape(5, -1, 2)
            fov_mask = fov_mask[:, ::down_scale, ::down_scale, ::down_scale].reshape(5, -1)
        except:
            projected_pix = projected_pix.reshape(1, self.bev_h, self.bev_w, self.bev_z, 2)
            fov_mask = fov_mask.reshape(1, self.bev_h, self.bev_w, self.bev_z)
            projected_pix = projected_pix[:, ::down_scale, ::down_scale, ::down_scale, :].reshape(1, -1, 2)
            fov_mask = fov_mask[:, ::down_scale, ::down_scale, ::down_scale].reshape(1, -1)
